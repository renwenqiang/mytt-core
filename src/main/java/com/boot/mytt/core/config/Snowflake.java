...
public class Snowflake {
    ...
    public synchronized long nextId() {
        ...
        // 移位并通过或运算拼到一起组成ID
        Long id = ((timestamp - startTimestamp) << timestampLeftShift) | (dataCenterId << dataCenterIdShift) | (workerId << workerIdShift) | sequence;

        if (null == id || id == 0L) {
            //throw new SnowflakeException("system is busy id is null or 0");
        }
        return id;
    }
